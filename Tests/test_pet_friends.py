...
def test_get_all_pets_with_valid_key(filter=''):
    ...
    _, auth_key = pf.get_api_key(valid_email,
                                 valid_password)  # На этом этапе, если мы успешно получили ключ api key, статус этого запроса нам не важен Поэтому мы ставим прочерк (нижнее подчёркивание).
...
def test_not_successful_update_another_user_pet_info(name='Чужой', animal_type='Чужой', age='20'):
    """Проверяем что нельзя обновить информацию питомца другого пользователя"""
    # Получаем ключ auth_key и список всех питомцев
    _, auth_key = pf.get_api_key(valid_email, valid_password)
    _, pets = pf.get_list_of_pets(auth_key, "")
    _, my_pets = pf.get_list_of_pets(auth_key, "my_pets")

    # Если список всех питомцев больше чем список моих питомцев, получаем ID чужого питомца и пробуем обновить имя, тип и возраст питомца другого пользователя
    if len(pets['pets']) > len(my_pets['pets']):
        try:
            status, result = pf.update_pet_info(auth_key, pets['pets'][-1]['id'], name, animal_type, age)
            # Проверяем что статус ответа == 400 и имя питомца не соответствует заданному
            assert status == 400
            assert '400' in result
            assert result['name'] != name
        except AssertionError as error:
            # Если все проверки прошли успешно, поднимаем новое исключение с требуемым текстом
            raise AssertionError(f"Managed to update another user's pet information, {error}")
    else:
        # если список всех питомцев пустой, то пишем текстом об отсутствии чужих питомцев
        raise Exception("No other users pets")


def test_add_new_simple_pet_with_valid_data(name='Васька', animal_type='жаба',
                                            age='4', ):
...
def test_must_not_add_pet_photo_with_incorrect_extension(pet_photo='images/manky.pdf'):
...
def test_add_pet_foto(pet_photo='images/Layga.jpg'):
...
def test_add_must_not_new_pet_with_not_valid_name(name='Рыся' * 10, animal_type='кошка',
                                                  age='4', pet_photo='images/Cat.jpeg'):
    """Проверяем что нельзя можно ли добавить питомца с именем содержащим большое количество букв."""
...
def test_must_not_add_new_pet_with_empty_strings(name='', animal_type='', age='', pet_photo='images/Cat.jpeg'):
...
def test_must_not_add_new_pet_with_not_valid_animal_type(name='Туман', animal_type=';$*/.,(}',
                                                         age='4', pet_photo='images/manky.jpeg'):
...
def test_must_not_add_new_pet_with_not_valid_age(name='Туман', animal_type='Собака',
                                                 age='четыре', pet_photo='images/manky.jpeg'):
...
def test_must_not_add_new_pet_with_not_valid_key(name='Туман', animal_type='Собака',
                                                 age='4', pet_photo='images/manky.jpeg'):
...
def test_sussefull_add_pet_photo_by_alien_id(pet_photo='images/manky.jpeg'):
    """Проверяем можно ли добавить фото питомца к чужому питомцу"""
    ...
    if len(pets['pets']) > len(my_pets['pets']):
        try:
            status, result = pf.add_pet_foto(auth_key, pets['pets'][-1]['id'], pet_photo)
            # Проверяем что статус ответа == 200 и фото добавлено
            assert status == 200
            assert 'pet_photo' in result
        except AssertionError as error:
            # Если все проверки прошли успешно, поднимаем новое исключение с требуемым текстом
            raise AssertionError(f"Photo not added, {error}")
    else:
        # если список всех питомцев пустой, то пишем текстом об отсутствии чужих питомцев
        raise Exception("No other users pets")
